...
class CorruptedMNISTDataset(Dataset):
    def __init__(self, train=True):
        ...
        files = [
            np.load(os.path.join(corrupt_path, x))
            for x in os.listdir(corrupt_path)
            if extension in x
        ]
...
def mnist(train=True):
    ...
    images = torch.from_numpy(np.concatenate([x["images"] for x in files], axis=0))
    labels = torch.from_numpy(np.concatenate([x["labels"] for x in files], axis=0))
